...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(c.getCount() == 0) {
            ...
            for(int i = 0; i < name.length; i++) {
                ...
                Log.d(LOG, "id = " + db.insert("mytable", null, cv));
            }
        }
        c.close();
        dbHelper.close();
    ...
    public void onClick(View v) {
        ...
        switch(v.getId()) {
            // Все записи
            case R.id.btnAll:
                Log.d(LOG, "--- Все записи ---");
                c = db.query("mytable", null, null, null,
                        null, null, null);
                break;
            // Функция
            case R.id.btnFunc:
                Log.d(LOG, "--- Функция " + sFunc + " ---");
            ...
            case R.id.btnPeople:
                Log.d(LOG, "--- Население больше " + sPeople + " ---");
                selection = "people > ?";
            ...
            case R.id.btnGroup:
                Log.d(LOG, "--- Население по региону ---");
            ...
            case R.id.btnHaving:
                Log.d(LOG, "--- Регионы с населением больше " + sRegionPeople + " ---");
                columns = new String[] { "region", "sum(people) as people"};
                groupBy = "region";
            ...
            case R.id.btnSort:
                ...
                switch(rgSort.getCheckedRadioButtonId()) {
                    // наименование
                    case R.id.rName:
                        Log.d(LOG, "--- Сортировка по наименованию ---");
                        orderBy = "name";
                        break;
                    // население
                    case R.id.rPeople:
                        Log.d(LOG, "--- Сортировка по населению ---");
                        orderBy = "people";
                        break;
                    // регион
                    case R.id.rRegion:
                        Log.d(LOG, "--- сортировка по региону ---");
                        orderBy = "region";
                        break;
                }
                c = db.query("mytable", null, null, null,
